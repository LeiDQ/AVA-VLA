...
python experiments/robot/aloha/preprocess_split_aloha_data.py \
  --dataset_path "$DATA_ROOT/aloha1_raw/put_green_pepper_into_pot/" \
  --out_base_dir "$DATA_ROOT/aloha1_preprocessed/" \
  --percent_val 0.05
python experiments/robot/aloha/preprocess_split_aloha_data.py \
  --dataset_path "$DATA_ROOT/aloha1_raw/put_red_pepper_into_pot/" \
  --out_base_dir "$DATA_ROOT/aloha1_preprocessed/" \
  --percent_val 0.05
python experiments/robot/aloha/preprocess_split_aloha_data.py \
  --dataset_path "$DATA_ROOT/aloha1_raw/put_yellow_corn_into_pot/" \
  --out_base_dir "$DATA_ROOT/aloha1_preprocessed/" \
  --percent_val 0.05
```
...
In the primary conda environment (`avavla`) which you will use to launch the VLA server, install a few packages for the server-client interface:
...
conda create -n avavla-aloha python=3.10 -y
conda activate avavla-aloha
...
Launch the VLA server on the machine that has the GPU you will use to run model inference (using the `avavla` conda environment). Below is a sample command for this (change as needed):
...
* Tip #1: If you run into a ROS error such as `ImportError: /lib/x86_64-linux-gnu/libp11-kit.so.0: undefined symbol: ffi_type_pointer, version LIBFFI_BASE_7.0`, try running the following command in your client conda environment (`avavla-aloha`):
