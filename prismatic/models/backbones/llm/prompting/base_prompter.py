...
class PurePromptBuilder(PromptBuilder):
    ...
    def __init__(self, model_family: str, system_prompt: Optional[str] = None, bos: Optional[str] = None, eos: Optional[str] = None) -> None:
        super().__init__(model_family, system_prompt)

        self.bos, self.eos = (
            (bos, eos)
            if bos is not None and eos is not None
            else self.DEFAULT_BOS_EOS.get(model_family, ("<s>", "</s>"))
        )
