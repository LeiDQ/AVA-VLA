...
def load_vla(
    ...
    if model_path.is_dir():
        overwatch.info(f"Loading from local path `{(run_dir := model_path)}`")

        config_json = run_dir / "config.json"
        dataset_statistics_json = run_dir / "dataset_statistics.json"
        checkpoint_dir = run_dir / "checkpoints"
        if step_to_load is None:
            checkpoint_pt = checkpoint_dir / "latest-checkpoint.pt"
        else:
            matched_checkpoints = list(checkpoint_dir.glob(f"*step-{step_to_load:06d}*.pt"))
            assert matched_checkpoints, f"No checkpoint found for step `{step_to_load}` in `{checkpoint_dir}`"
            assert len(matched_checkpoints) == 1, (
                f"Multiple checkpoints matched step `{step_to_load}` in `{checkpoint_dir}`; specify a unique file path instead."
            )
            checkpoint_pt = matched_checkpoints[0]

        assert config_json.exists(), f"Missing `config.json` for `{run_dir = }`"
        assert dataset_statistics_json.exists(), f"Missing `dataset_statistics.json` for `{run_dir = }`"
    elif model_path.is_file():
        overwatch.info(f"Loading from local checkpoint path `{(checkpoint_pt := model_path)}`")
