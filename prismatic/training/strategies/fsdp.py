    ...
    def save_checkpoint(
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy):
            ...
            if overwatch.is_rank_zero():
                ...
                # Save Checkpoint & update `latest-checkpoint.pt` if possible.
                torch.save({"model": model_state_dicts}, checkpoint_path)
                try:
                    shutil.copy(checkpoint_path, checkpoint_dir / "latest-checkpoint.pt")
                except (PermissionError, OSError) as exc:
                    overwatch.warning(
                        f"Could not copy latest checkpoint due to permissions or filesystem issues: {exc}. "
                        "Continuing without `latest-checkpoint.pt`."
                    )
