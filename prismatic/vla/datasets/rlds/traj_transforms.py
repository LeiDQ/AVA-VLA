...
def chunk_act_obs(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    ...
    # Preserve optional task-level signals so AVA-VLA can train reasoning from environment rewards when present.
    for key in ("reward", "discount", "is_first", "is_last", "is_terminal", "success"):
        if key in traj:
            traj[key] = tf.gather(traj[key], floored_action_chunk_indices)
