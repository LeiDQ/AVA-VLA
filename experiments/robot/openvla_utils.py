...
# Apply JSON numpy patch for serialization when the optional helper is installed.
try:
    import json_numpy
...
def resize_image_for_policy(img: np.ndarray, resize_size: Union[int, Tuple[int, int]]) -> np.ndarray:
    ...
    if tf is None:
        return np.array(Image.fromarray(img).resize((resize_size[1], resize_size[0]), Image.Resampling.LANCZOS))
...
def crop_and_resize(image: tf.Tensor, crop_scale: float, batch_size: int) -> tf.Tensor:
    ...
    """
    if tf is None:
        raise RuntimeError("TensorFlow is required for crop_and_resize; install LIBERO/OpenVLA eval requirements.")
...
def center_crop_image(image: Union[np.ndarray, Image.Image]) -> Image.Image:
    ...
    if tf is None:
        ...
        return cropped.resize((OPENVLA_IMAGE_SIZE, OPENVLA_IMAGE_SIZE), Image.Resampling.LANCZOS).convert("RGB")
