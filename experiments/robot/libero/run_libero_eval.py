...
class GenerateConfig:
    ...
    use_history_state: bool = True                   # (For AVA-VLA) Carry h_{t-1} between policy queries
    log_reasoning_info: bool = False                 # (For AVA-VLA) Print reasoning diagnostics
...
def initialize_model(cfg: GenerateConfig):
    ...
    if cfg.model_family == "avavla":
        check_unnorm_key(cfg, model)
        return model, getattr(model, "action_head", None), None, None, None
...
def process_action(action, model_family):
    ...
    # OpenVLA-style dataloaders flip the gripper sign to align with other datasets
    # (0 = close, 1 = open), so flip it back (-1 = open, +1 = close) before executing the action
    if model_family in {"openvla", "avavla"}:
