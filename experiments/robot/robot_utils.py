...
from vla_scripts.deploy_avavla import (
    load_avavla_model,
    predict_action as predict_avavla_action,
)
    ...
    with torch.no_grad():
        if cfg.model_family == "openvla":
            ...
            if getattr(cfg, "log_reasoning_info", False):
                print(
                    "AVA-VLA reasoning: "
                    f"steps={reasoning_info.get('num_steps_performed', 0)}, "
                    f"exit={reasoning_info.get('final_exit_scores', None)}"
                )
            action = [actions[i] for i in range(len(actions))] if np.asarray(actions).ndim > 1 else [actions]
        else:
            raise ValueError(f"Unsupported model family: {cfg.model_family}")
